...
def kalman_filter_step(belief, control, measurement):
    ...
    # --->>> Put your code here.
    a =1
    c =1 # Conversion factors set to default value 1
    # Prediction.
    predicted_mu = a*belief.mu + control.mu
    predicted_sigma2 = (a**2)*belief.sigma2 + control.sigma2
    prediction = Density(predicted_mu, predicted_sigma2)

    # Correction.
    kalman_gain = c*predicted_sigma2/((c**2)*predicted_sigma2+measurement.sigma2)
    corrected_mu = predicted_mu+kalman_gain*(measurement.mu-c*predicted_mu)
    corrected_sigma2 = (1-kalman_gain*c)*predicted_sigma2
    correction = Density(corrected_mu, corrected_sigma2)
