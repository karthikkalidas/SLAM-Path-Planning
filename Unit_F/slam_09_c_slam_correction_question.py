...
class ExtendedKalmanFilterSLAM:
    ...
    def predict(self, control):
        ...
        Gf = zeros((3+2*N,3+2*N))
        Gf[0:3,0:3] += G3
        Gf[3:,3:] += eye(2*N)
        Rf = zeros((3+2*N,3+2*N))
        Rf[0:3,0:3] += R3

        self.covariance = dot(Gf, dot(self.covariance, Gf.T)) + Rf
    ...
    def add_landmark_to_state(self, initial_coords):
        ...
        new_covariance = zeros((dim_x+2, dim_y+2))
        new_covariance[0:dim_x, 0:dim_y] = self.covariance
        new_covariance[dim_x:, dim_y:] = diag([sigma_x**2, sigma_y**2])
    ...
    def correct(self, measurement, landmark_index):
        ...
        H = zeros((2,3+2*N))
        H[:,0:3] = H3
        H[:,3+2*landmark_index:3+2*landmark_index+2] = -H3[:,0:2]
