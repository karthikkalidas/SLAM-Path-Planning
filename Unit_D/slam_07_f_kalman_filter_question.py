...
class ExtendedKalmanFilter:
    ...
    def predict(self, control):
        ...
        sigma_l2 = (alpha1*l)**2 + (alpha2*(l-r))**2
        sigma_r2 = (alpha1*r)**2 + (alpha2*(l-r))**2
        covariance_control = diag([sigma_l2,sigma_r2])
        G = self.dg_dstate(self.state, control, self.robot_width)
        V = self.dg_dcontrol(self.state, control, self.robot_width)
        self.covariance = dot(dot(G,self.covariance),G.T) + dot(dot(V,covariance_control),V.T)
    ...
    def dh_dstate(state, landmark, scanner_displacement):
        ...
        delr_delx = -delta_x/r
        delr_dely = -delta_y/r
        delr_deltheta = d*(delta_x*sin(theta)-delta_y*cos(theta))/r

        delalpha_delx = delta_y/(r**2)
        delalpha_dely = -delta_x/(r**2)
        delaplha_deltheta = -(d*(delta_x*cos(theta) + delta_y*sin(theta))/(r**2)) - 1

        return array([[delr_delx, delr_dely, delr_deltheta], [delalpha_delx, delalpha_dely, delaplha_deltheta]])
    ...
    def correct(self, measurement, landmark):
        ...
        sigma2_alpha = self.measurement_angle_stddev**2
        Q = diag([sigma2_r,sigma2_alpha])
        K = dot(self.covariance, dot(H.T,linalg.inv(dot(H,dot(self.covariance, H.T))+Q)))
        innovation = measurement - self.h(self.state, landmark, scanner_displacement)
