...
class Particle:
    ...
    def h_expected_measurement_for_landmark(self, landmark_number,
        ...
        h_exp = self.h(self.pose, self.landmark_positions[landmark_number], scanner_displacement)
        return h_exp
    ...
    def wl_likelihood_of_correspondence(self, measurement,
        ...
        Ql = self.H_Ql_jacobian_and_measurement_covariance_for_landmark(landmark_number, Qt_measurement_covariance, scanner_displacement)[1]
        likelihood = (1/(2*pi*sqrt(np.linalg.det(Ql)))) *exp(-0.5*np.dot(np.dot(delta_z.T,np.linalg.inv(Ql)),delta_z))
        return likelihood
    ...
    def update_landmark(self, landmark_number, measurement,
        ...
        expected_measurement_for_landmark = self.h_expected_measurement_for_landmark(landmark_number, scanner_displacement)
        delta_z = measurement - expected_measurement_for_landmark
        ...
        self.landmark_covariances[landmark_number] = np.dot(np.eye(KH.shape[0]) - KH ,self.landmark_covariances[landmark_number])
    ...
    def update_particle(self, measurement, measurement_in_scanner_system,
        ...
        likelihoods = self.compute_correspondence_likelihoods(measurement, n, Qt_measurement_covariance, scanner_displacement)
    ...
    def decrement_visible_landmark_counters(self, scanner_displacement):
        ...
        for landmark in xrange(self.number_of_landmarks()):
            bearing = self.h_expected_measurement_for_landmark(landmark, scanner_displacement)[1]
            if bearing>=min_bearing and bearing<=max_bearing:
                self.landmark_counters[landmark]-=1
        # Hints:
    ...
    def remove_spurious_landmarks(self):
        ...
        for i in xrange(len(self.landmark_counters)):
            if self.landmark_counters[i] >= 0:
                new_landmark_positions.append(self.landmark_positions[i])
                new_landmark_covariances.append(self.landmark_covariances[i])
                new_landmark_counters.append(self.landmark_counters[i])
