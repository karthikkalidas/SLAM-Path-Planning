...
class Particle:
    ...
    def wl_likelihood_of_correspondence(self, measurement,
        ...
        l = (1/(2*pi*sqrt(np.linalg.det(Ql)))) *exp(-0.5*np.dot(np.dot(delta_z.T,np.linalg.inv(Ql)),delta_z))
        return l
    ...
    def update_particle(self, measurement, measurement_in_scanner_system,
        ...
        # Else update the particle's EKF for the corresponding particle.
            ...
            w, index = max(likelihoods), np.argmax(likelihoods)  # Replace this by a search for max, argmax.
